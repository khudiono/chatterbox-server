...
describe('Node Server Request Listener Function', function() {
  ...
  it('Should accept posts to /classes/messages', function() {
    ...
    expect(JSON.parse(res._data).username).to.equal(stubMsg.username);
    expect(JSON.parse(res._data).objectId).to.exist;
    expect(JSON.parse(res._data).createdAt).to.exist;
    expect(res._ended).to.equal(true);
  });
  ...
  it('Should not duplicate object IDs', function() {
    var stubMsg = {
      username: 'Jono',
      text: 'Do my bidding!'
    };
    ...
    expect(res._responseCode).to.equal(201);
    expect(res._ended).to.equal(true);
    expect(JSON.parse(res._data).objectId).to.not.equal(JSON.parse(res2._data).objectId)
  });
